...
from config import logger, shared_state
...
def safe_tool(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            logger.debug(f" 调用本地工具 [{tool_name}]")
...
def write_email(to: str, subject: str, content: str, cc: str = "", draft_only: bool = False) -> str:
    ...
    :param subject: 邮件主题
    :param content: 邮件正文
    :param draft_only: 如果为True，只生成草稿不发送；如果为False，立刻发送。
    """
    
    smtp_user = os.getenv("SMTP_USER", "")
    if not smtp_user:
        return " 环境变量未配置 SMTP_USER，无法操作邮件。"
    ...
    if draft_only:
        ...
        if not email_password:
            return " 未配置 EMAIL_PASSWORD，无法连接服务器保存草稿。"
            
        try:
            mail = imaplib.IMAP4_SSL(imap_server)
            mail.login(smtp_user, email_password)
            ...
            return f" 智能体草稿生成成功，但同步到邮箱服务器失败: {e}。内容已保留在内存中。"
